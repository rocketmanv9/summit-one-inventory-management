-- Per-tenant control over what Isabelle (the AI agent) is allowed to DO.
--
-- agent_permissions is a jsonb map of capability -> level:
--   off  — the agent refuses and tells the user it's disabled in Settings.
--   ask  — the agent previews the action and only runs it after the user confirms.
--   auto — the agent performs the action immediately, no confirmation.
--
-- Capabilities:
--   stock_adjust    — set/adjust on-hand quantities
--   stock_issue     — issue/release stock to jobs, trucks, people
--   transfer        — move stock between locations
--   reserve         — reserve stock / release reservations
--   create_records  — create vendors, items, locations, categories, assets
--   purchase_orders — create draft purchase orders
--
-- Defaults preserve today's behavior: confirm-gated for stock-moving actions,
-- automatic for record creation.

ALTER TABLE supply_chain.tenant_settings
  ADD COLUMN IF NOT EXISTS agent_permissions jsonb NOT NULL DEFAULT jsonb_build_object(
    'stock_adjust', 'ask',
    'stock_issue', 'ask',
    'transfer', 'ask',
    'reserve', 'ask',
