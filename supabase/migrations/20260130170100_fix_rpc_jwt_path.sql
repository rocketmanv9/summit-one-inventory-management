-- Fix RPC to read tenant_id from correct JWT path
-- The tenant_id is stored in app_metadata.tenant_id, not at the root level
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(
      CASE 
        ...
        THEN pol.quantity_ordered * pol.unit_price 
        ELSE 0 
      END
  ...
  LEFT JOIN supply_chain.purchase_order_lines pol ON po.id = pol.purchase_order_id
  WHERE po.tenant_id = v_tenant_id
    AND po.status IN ('pending', 'approved', 'partially_received')
