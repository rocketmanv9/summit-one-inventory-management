-- 20260814000011_wizard_create_item_explicit_tenant.sql
--
-- Let Isabelle's conversational procure loop add a brand-new item mid-chat.
--
-- The chat SSE loop runs server tools with a tenant SERVICE client whose JWT
-- carries no tenant/user claims, so inventory.rpc_wizard_create_item (and the
-- inventory.rpc_create_catalog_item it calls) hit `current_tenant_id() IS NULL`
-- and raise "Authentication required". This is the same GUC/pooling gotcha the
-- purchasing path already solved by accepting an explicit p_tenant_id on the
-- service-role branch (supply_chain.rpc_create_purchase_order).
--
-- Fix: add an OPTIONAL p_tenant_id (+ p_acting_user_id on the wizard) to both
-- RPCs. When the caller is service_role, prefer the explicit value, falling back
-- to JWT claims; the normal authenticated path is unchanged (current_tenant_id()).
-- Additive, backward-compatible: every existing call keeps working because the
-- new params default to NULL and existing signatures are preserved by name.
--
-- Stage-only, additive. Mirrors the established explicit-p_tenant_id pattern.
--
-- NOTE: CREATE OR REPLACE cannot add a parameter to an existing function — it
-- would create an overload and make named-arg calls ambiguous. So we DROP the
-- exact old signatures first, then recreate with the new trailing param(s).
-- Every caller uses named args and the new params default to NULL, so this is
-- transparent to existing callers.

DROP FUNCTION IF EXISTS inventory.rpc_create_catalog_item(
  text, text, uuid, text, numeric, text, text, text, uuid
);
DROP FUNCTION IF EXISTS inventory.rpc_wizard_create_item(
  text, text, text, numeric, text, text, uuid, jsonb, uuid, jsonb, text, numeric,
  uuid, jsonb, numeric, numeric, text, text, text, boolean, text[], jsonb, uuid
);

-- ── inventory.rpc_create_catalog_item — add optional p_tenant_id ─────────────
CREATE OR REPLACE FUNCTION inventory.rpc_create_catalog_item(
  ...
  p_uom_term_id uuid DEFAULT NULL::uuid,
...
BEGIN
  -- Service-role callers (the AI chat loop) pass tenant explicitly; everyone
  -- else keeps deriving it from the session JWT.
...
-- ── inventory.rpc_wizard_create_item — add optional p_tenant_id / p_acting_user_id ──
CREATE OR REPLACE FUNCTION inventory.rpc_wizard_create_item(
  p_name text,
  p_description text DEFAULT NULL::text,
  p_tracking_mode text DEFAULT 'stock'::text,
  p_reorder_point numeric DEFAULT NULL::numeric,
  p_base_sku text DEFAULT NULL::text,
  p_sku text DEFAULT NULL::text,
  p_category_id uuid DEFAULT NULL::uuid,
  p_create_category jsonb DEFAULT NULL::jsonb,
  p_vendor_id uuid DEFAULT NULL::uuid,
  p_create_vendor jsonb DEFAULT NULL::jsonb,
  p_vendor_sku text DEFAULT NULL::text,
  p_vendor_unit_cost numeric DEFAULT NULL::numeric,
  p_location_id uuid DEFAULT NULL::uuid,
  p_create_location jsonb DEFAULT NULL::jsonb,
  p_initial_qty numeric DEFAULT NULL::numeric,
  p_initial_cost numeric DEFAULT NULL::numeric,
  p_barcode text DEFAULT NULL::text,
  p_create_assets text DEFAULT NULL::text,
  p_idempotency_key text DEFAULT NULL::text,
  p_has_variants boolean DEFAULT false,
  p_variant_dimensions text[] DEFAULT NULL::text[],
  p_variant_options jsonb DEFAULT NULL::jsonb,
  p_uom_term_id uuid DEFAULT NULL::uuid,
  p_tenant_id uuid DEFAULT NULL::uuid,
...
BEGIN
  -- Service-role callers (the AI chat loop) pass tenant/actor explicitly because
  -- their JWT carries no claims; the authenticated path is unchanged.
  IF (auth.jwt()->>'role') = 'service_role' THEN
    v_tenant_id := COALESCE(
      p_tenant_id,
      (auth.jwt()->'app_metadata'->>'tenant_id')::uuid,
  ...
  END IF;
  -- Pass tenant through explicitly so the inner catalog-item RPC resolves the
  -- same tenant on the service-role path (it also reads current_tenant_id()).
  SELECT * INTO v_item_result FROM inventory.rpc_create_catalog_item(p_name:=p_name,p_description:=p_description,p_category_id:=v_category_id,p_tracking_mode:=p_tracking_mode,p_reorder_point:=p_reorder_point,p_base_sku:=p_base_sku,p_sku:=p_sku,p_last_event_id:=p_idempotency_key,p_uom_term_id:=p_uom_term_id,p_tenant_id:=v_tenant_id);
