-- Fix rpc_inv_apply_ops_equipment_hold: the one-active-reservation-per-asset
-- guard (idx_reservations_active_asset) is a UNIQUE INDEX, so collisions
-- raise unique_violation — which the old function did not catch, so every
-- upsert against an asset with a stale active mirror row crashed (silently
-- stalling the Operations mirror since 2026-07-20). Now: stale ops-sourced
-- active rows on the same asset are SUPERSEDED (released) before writing —
-- a re-keyed ops hold (release + re-reserve mints a new assignment id) is
-- the normal case; a NON-ops manual reservation keeps the asset and the ops
-- write yields with outcome 'conflict_manual_reservation'. Both
-- unique_violation and exclusion_violation are caught as belt-and-braces.
-- (Applied to stage 2026-07-28 via MCP as 'ops_hold_mirror_supersede_stale';
-- the 8 days of missed ops holds were reconciled the same day.)
