-- Create catalog items with atomic SKU allocation
...
BEGIN
  ...
  IF p_category_id IS NULL THEN
    ...
    INSERT INTO inventory.catalog_items (
      ...
      p_base_sku,
      p_last_event_id
    ) RETURNING inventory.catalog_items.id, inventory.catalog_items.sku, inventory.catalog_items.base_sku, inventory.catalog_items.last_event_id
      INTO id, sku, base_sku, last_event_id;
...
GRANT EXECUTE ON FUNCTION inventory.rpc_create_catalog_item(text, text, uuid, text, text, numeric, text, text, text) TO authenticated;
