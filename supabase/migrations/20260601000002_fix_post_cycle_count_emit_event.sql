-- Fix: post_cycle_count_adjustments called public.emit_event with parameter
-- names that don't exist (p_event_name, p_actor_user_id), so posting a cycle
-- count threw: "function public.emit_event(...) does not exist". Approving works
-- because rpc_inv_cycle_count_approve emits via inventory.publish_event; switch
-- the post function to the same proven path so approve-and-post completes.
...
BEGIN
    ...
    -- Emit via inventory.publish_event (same path rpc_inv_cycle_count_approve
    -- uses) instead of the non-existent public.emit_event(p_event_name, ...).
    PERFORM inventory.publish_event(
        p_tenant_id => p_tenant_id,
        p_scope => 'inventory',
        p_event_type => 'cycle_count.posted',
