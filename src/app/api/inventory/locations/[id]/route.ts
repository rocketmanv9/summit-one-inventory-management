...
export const PATCH = updateRouteOCC({
  ...
  // Same address enforcement as create — a bad state/ZIP can't be saved via edit either.
  bodySchema: locationAddressSchema(
    z.object({ expected_last_event_id: z.string().min(1) }).passthrough(),
  ),
});
