...
export function OnboardingQRDialog({
  isOpen,
  onClose,
  locationId,
  locationName,
}: OnboardingQRDialogProps) {
  ...
  const [activeSessions, setActiveSessions] = useState<OnboardingSession[]>([]);
  ...
  const fetchActiveSessions = async () => {
    try {
      const res = await authenticatedFetch('/api/inventory/onboarding');
  ...
  const generateSession = async () => {
    ...
    try {
      const res = await apiWrite('/api/inventory/onboarding', {
        method: 'POST',
        body: { location_id: locationId, ttl_minutes: ttl },
      });
  ...
  const revokeSession = async (sessionId: string) => {
    try {
      const res = await apiWrite(`/api/inventory/onboarding/${sessionId}`, {
        method: 'DELETE',
      });
        ...
        <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto z-50 p-6">
          <div className="flex items-center justify-between mb-4">
            <Dialog.Title className="text-lg font-semibold">
              Onboard Inventory - {locationName}
            </Dialog.Title>
          ...
          <Dialog.Description className="text-sm text-gray-600 mb-6">
            Generate a QR code to set up initial stock at this location from a mobile device.
          </Dialog.Description>
          ...
          {!qrDataUrl ? (
            ...
            <div className="space-y-4">
              ...
              <div className="text-center text-sm text-gray-600">
                Scan this QR code with your phone to start onboarding
              </div>
                  ...
                  >
                    <div>
                      <div className="font-medium">
                        {s.location_name || 'Unknown'} - Created {formatTime(s.created_at)}
                      </div>
